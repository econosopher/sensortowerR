...
#' Useful for finding valid inputs for the `category` parameter in other functions.
#'
...
get_categories <- function(platform = NULL) { # Renamed function
  ...
  if (!is.null(platform)) {
    ...
    if (!platform %in% c("ios", "android")) {
      warning("Invalid platform specified. Choose 'ios' or 'android'. Returning all categories.", call. = FALSE)
    } else {
